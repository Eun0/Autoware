...
void PlanningHelpers::CalculateRollInTrajectoriesIrregular(const WayPoint& carPos, const double& speed, const vector<WayPoint>& originalCenter, int& start_index,
    ...
    std::vector<WayPoint>& sampledPoints)
{
  ...
  for(int i=0; i< rollOutNumber+1; i++)
    {
      double density=rollOutDensity;
      if(i - centralTrajectoryIndex<0)
        density=rollOutDensity*1.3;
      double end_roll_in_distance = density*(i - centralTrajectoryIndex);
